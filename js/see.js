...
class MoviePage {
    ...
    createAdOverlay(containerSelector) {
        ...
        // Оверлей будет больше на 20px с каждой стороны (гарантированно перекрывает всё)
        const padding = 20;
        
        this.adOverlay = document.createElement('div');
        this.adOverlay.id = 'adOverlay';
        
        // Используем clip-path для обрезки оверлея по размерам модального окна
        const clipPath = `inset(0px round 12px)`;
        
        this.adOverlay.style.cssText = `
            position: fixed;
            top: ${containerRect.top - padding}px;
            left: ${containerRect.left - padding}px;
            width: ${containerRect.width + padding * 2}px;
            height: ${containerRect.height + padding * 2}px;
        ...
        const updatePosition = () => {
            ...
            this.adOverlay.style.top = (newRect.top - padding) + 'px';
            this.adOverlay.style.left = (newRect.left - padding) + 'px';
            this.adOverlay.style.width = (newRect.width + padding * 2) + 'px';
            this.adOverlay.style.height = (newRect.height + padding * 2) + 'px';
        };
