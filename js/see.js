...
class MoviePage {
    ...
    async loadViaTmdb() {
        ...
        try {
            ...
            document.title = `Смотреть ${movieTitle}${movieYear ? ` (${movieYear})` : ''} бесплатно | KKpoisk`;
    ...
    async loadLogo() {
        const heroLogoResponse = await fetch(`${API_BASE}?tmdb=/${this.contentType}/${this.tmdbId}/images`);
        if (heroLogoResponse.ok) {
            ...
            if (heroLogoData.logos && heroLogoData.logos.length > 0) {
                ...
                for (const logo of heroLogoData.logos) {
                    if (logo.iso_639_1 && logo.iso_639_1.includes('ru')) { selectedLogo = logo; break; }
                }
                if (!selectedLogo) {
                    for (const logo of heroLogoData.logos) {
                        if (logo.iso_639_1 && logo.iso_639_1.includes('en')) { selectedLogo = logo; break; }
                    }
                }
                if (!selectedLogo) {
                    for (const logo of heroLogoData.logos) {
                        if (!logo.iso_639_1 || logo.iso_639_1 === '') { selectedLogo = logo; break; }
                    }
                }
                if (!selectedLogo && heroLogoData.logos.length > 0) selectedLogo = heroLogoData.logos[0];
                if (selectedLogo) this.logoUrl = selectedLogo.file_path;
            }
        }
    }
    ...
    renderEpisodesGrid() {
        ...
        for (const episode of episodes) {
            ...
            // Название в формате: "1 серия: Название"
            const fullTitle = `${episodeNum} серия: ${episodeName}`;
            
            // Длительность в правом нижнем углу как бейдж
            let durationBadge = '';
