...
const AD_IMAGE_URL = '/imgs/subscribe.jpg'; // Путь к картинке рекламы
...
const API_CONFIG = {
    alloha: {
        getUrl: (tmdbId, posterPath, season, episode) => {
            ...
            if (posterPath) {
                const posterUrl = `${window.location.origin}/api/proxy?image=${encodeURIComponent(posterPath)}&size=original`;
...
class MoviePage {
    ...
    async loadViaTmdb() {
        ...
        try {
            ...
            if (!tmdbResponse.ok) { this.showError(`Не удалось получить данные`); return; }
        ...
        } catch (error) { console.error(error); this.showError('Произошла ошибка при загрузке.'); }
    }
    ...
    renderCastSection() {
        ...
        for (const actor of this.castData) {
            ...
            if (profilePath) {
                const profileUrl = `${API_BASE}?image=${encodeURIComponent(profilePath)}&size=w500`;
            ...
            if (filtered.length > 0) {
                ...
                for (const movie of filtered) {
                    ...
                    if (movie.poster_path) {
                        const posterUrl = `${API_BASE}?image=${encodeURIComponent(movie.poster_path)}&size=w500`;
    ...
    setupPoster() {
        return new Promise((resolve) => {
            ...
            if (isMobile) {
                if (this.posterWithoutLang) imageUrl = `${API_BASE}?image=${encodeURIComponent(this.posterWithoutLang)}&size=original`;
                else if (this.bestBackdrop) imageUrl = `${API_BASE}?image=${encodeURIComponent(this.bestBackdrop)}&size=original`;
                else if (this.tmdbData?.backdrop_path) imageUrl = `${API_BASE}?image=${encodeURIComponent(this.tmdbData.backdrop_path)}&size=original`;
            } else {
                if (this.bestBackdrop) imageUrl = `${API_BASE}?image=${encodeURIComponent(this.bestBackdrop)}&size=original`;
                else if (this.tmdbData?.backdrop_path) imageUrl = `${API_BASE}?image=${encodeURIComponent(this.tmdbData.backdrop_path)}&size=original`;
                else if (this.posterWithoutLang) imageUrl = `${API_BASE}?image=${encodeURIComponent(this.posterWithoutLang)}&size=original`;
            }
    ...
    renderEpisodesGrid() {
        ...
        for (const episode of episodes) {
            ...
            let posterHtml = '';
            if (imagePath) {
                const imageUrl = `${API_BASE}?image=${encodeURIComponent(imagePath)}&size=original`;
    ...
    // ===== РЕКЛАМНЫЙ БАНЕР =====
    createAdModal() {
        if (this.adModal) return this.adModal;
        
        this.adModal = document.createElement('div');
        this.adModal.id = 'adModal';
        ...
        this.adModal.innerHTML = `
            <div style="position: relative; max-width: 90vw; max-height: 90vh; cursor: pointer;" onclick="window.moviePage.openAdLink()">
                <img src="${AD_IMAGE_URL}" alt="Реклама" style="max-width: 100%; max-height: 85vh; border-radius: 12px; box-shadow: 0 10px 40px rgba(0,0,0,0.5); display: block;">
                
                <!-- Таймер 15 секунд в левом верхнем углу -->
                ...
                <!-- Кнопка "Подписаться" в правом верхнем углу -->
                ...
                <!-- Кнопка "Пропустить" в правом нижнем углу -->
        ...
        // Обработчики
        const skipBtn = this.adModal.querySelector('#adSkipBtn');
        const subscribeBtn = this.adModal.querySelector('#adSubscribeBtn');
        const timerEl = this.adModal.querySelector('#adTimer');
        ...
        this.adTimer = setInterval(() => {
            ...
            if (this.adCountdown <= 0) {
                ...
                this.adTimer = null;
                // Автоматически закрываем банер через 15 секунд
                this.closeAdModal();
            }
        }, 1000);
        
        // Таймер для кнопки "Пропустить" (3 секунды)
        let skipCounter = 3;
    ...
    showAdModal() {
        this.createAdModal();
        // Показываем банер с плавным появлением
        requestAnimationFrame(() => {
            this.adModal.style.opacity = '1';
            this.adModal.style.visibility = 'visible';
        });
        document.body.style.overflow = 'hidden';
    }
    ...
    closeAdModal() {
        ...
        this.adModal.style.opacity = '0';
        this.adModal.style.visibility = 'hidden';
        document.body.style.overflow = '';
        
        // После исчезновения показываем плеер
        setTimeout(() => {
            if (this.adModal) {
                this.adModal.style.display = 'none';
            }
            // Показываем основной плеер
            this.showPlayerAfterAd();
        }, 500);
    }
    ...
    openEpisodePlayer(season, episode) {
        ...
        // Показываем рекламный банер
        this.showAdModal();
        
        // Подготавливаем плеер, но не показываем его пока
        const modal = document.getElementById('player-modal');
        if (!modal) return;
        ...
        if (logoContainer) {
            if (this.logoUrl) {
                const logoUrl = `${API_BASE}?image=${encodeURIComponent(this.logoUrl)}&size=original`;
        ...
        // Убираем модальное окно, чтобы не мешать банеру
        modal.classList.remove('active');
        
        // Обработчики для закрытия плеера
    ...
    openTrailerModal() {
        ...
        if (this.tmdbData && this.tmdbData.backdrop_path) {
            const fullPosterUrl = `${window.location.origin}/api/proxy?image=${encodeURIComponent(this.tmdbData.backdrop_path)}&size=original`;
            posterParam = `&poster=${encodeURIComponent(fullPosterUrl)}`;
        } else if (this.tmdbData && this.tmdbData.poster_path) {
            const fullPosterUrl = `${window.location.origin}/api/proxy?image=${encodeURIComponent(this.tmdbData.poster_path)}&size=original`;
            posterParam = `&poster=${encodeURIComponent(fullPosterUrl)}`;
        }
        
        let logoUrl = null;
        if (this.logoUrl) logoUrl = `${API_BASE}?image=${encodeURIComponent(this.logoUrl)}&size=original`;
    ...
    hideLoadingScreen() {
        ...
        if (screen && !this.loadError) {
            ...
            setTimeout(() => { screen.style.display = 'none'; screen.classList.remove('fade-out'); this.showMainContent(); }, 500);
        }
    }
    
    showMainContent() { document.getElementById('main-content').style.display = 'block'; }
}

document.addEventListener('DOMContentLoaded', () => { window.moviePage = new MoviePage(); });
