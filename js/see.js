...
class MoviePage {
    ...
    createAdOverlay(containerSelector) {
        ...
        // Получаем размеры и стили контейнера
        const containerRect = container.getBoundingClientRect();
        const containerStyle = window.getComputedStyle(container);
        const borderRadius = containerStyle.borderRadius || '12px';
        const transform = containerStyle.transform || 'scale(1)';
        const maxWidth = containerStyle.maxWidth || '85%';
        const maxHeight = containerStyle.maxHeight || '85%';
        const width = containerStyle.width || '100%';
        ...
        this.adOverlay.innerHTML = `
            <div style="position: relative; width: 100%; height: 100%; display: flex; justify-content: center; align-items: center; cursor: pointer; pointer-events: auto;" onclick="window.moviePage.openAdLink()">
                <img src="${AD_IMAGE_URL}" alt="Реклама" style="width: 100%; height: 100%; object-fit: contain; display: block;">
        ...
        document.body.appendChild(this.adOverlay);
        ...
        const updatePosition = () => {
            if (!this.adOverlay || this.adClosed) return;
            const newRect = container.getBoundingClientRect();
            const newStyle = window.getComputedStyle(container);
            this.adOverlay.style.top = newRect.top + 'px';
            this.adOverlay.style.left = newRect.left + 'px';
            this.adOverlay.style.width = newRect.width + 'px';
            this.adOverlay.style.height = newRect.height + 'px';
            this.adOverlay.style.borderRadius = newStyle.borderRadius || '12px';
            this.adOverlay.style.transform = newStyle.transform || 'scale(1)';
            this.adOverlay.style.maxWidth = newStyle.maxWidth || '85%';
            this.adOverlay.style.maxHeight = newStyle.maxHeight || '85%';
        };
        
        this._updateAdPosition = updatePosition;
    ...
    closeAdOverlay() {
        ...
        if (this._updateAdPosition) {
            window.removeEventListener('resize', this._updateAdPosition);
            window.removeEventListener('scroll', this._updateAdPosition);
            this._updateAdPosition = null;
        }
