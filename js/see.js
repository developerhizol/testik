...
const AD_OVERLAY_SIZE = 10; // ← Увеличивает оверлей на это количество пикселей по ВСЕМ краям
...
class MoviePage {
    ...
    createAdOverlay(containerSelector) {
        ...
        const size = AD_OVERLAY_SIZE || 0; // ← используем константу
        
        this.adOverlay = document.createElement('div');
        this.adOverlay.id = 'adOverlay';
        this.adOverlay.style.cssText = `
            position: fixed;
            top: ${containerRect.top - size}px;
            left: ${containerRect.left - size}px;
            width: ${containerRect.width + size * 2}px;
            height: ${containerRect.height + size * 2}px;
        ...
        const updatePosition = () => {
            ...
            this.adOverlay.style.top = (newRect.top - size) + 'px';
            this.adOverlay.style.left = (newRect.left - size) + 'px';
            this.adOverlay.style.width = (newRect.width + size * 2) + 'px';
            this.adOverlay.style.height = (newRect.height + size * 2) + 'px';
        };
