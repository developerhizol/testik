...
class MoviePage {
    ...
    showAdOverlay(containerSelector) {
        ...
        if (this.adOverlay) {
            // Даём небольшую задержку для появления оверлея синхронно с модальным окном
            setTimeout(() => {
    ...
    openEpisodePlayer(season, episode) {
        ...
        // Показываем рекламный оверлей СРАЗУ после открытия модального окна
        // с небольшой задержкой для синхронного появления
        setTimeout(() => {
    ...
    openTrailerModal() {
        ...
        // Показываем рекламный оверлей СРАЗУ после открытия модального окна трейлера
        setTimeout(() => {
