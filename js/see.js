...
class MoviePage {
    ...
    createAdOverlay(containerSelector) {
        ...
        // Устанавливаем overflow: hidden на контейнер, чтобы обрезать оверлей
        container.style.overflow = 'hidden';
        // Сохраняем ссылку на оригинальный overflow для восстановления
        this._originalOverflow = originalOverflow;
        
        // Создаём оверлей внутри контейнера
        this.adOverlay = document.createElement('div');
        this.adOverlay.id = 'adOverlay';
        
        // Оверлей чуть больше контейнера, но обрезается overflow: hidden
        const padding = 15;
    ...
    closeAdOverlay() {
        ...
        if (this.adOverlay) {
            ...
            setTimeout(() => {
                ...
                // Восстанавливаем overflow контейнера
                if (this.adParentContainer) {
                    this.adParentContainer.style.overflow = this._originalOverflow || '';
                }
            }, 500);
        }
    }
