...
class MoviePage {
    ...
    createAdOverlay(containerSelector) {
        ...
        // Получаем точные размеры и позицию контейнера
        const containerRect = container.getBoundingClientRect();
        
        // Растягиваем по горизонтали (убираем зазоры по бокам)
        // По вертикали оставляем точно по размеру (убираем вылезание)
        const horizontalPadding = 8; // растягиваем по бокам
        const verticalPadding = 0; // не добавляем по вертикали
        
        this.adOverlay = document.createElement('div');
        this.adOverlay.id = 'adOverlay';
        this.adOverlay.style.cssText = `
            position: fixed;
            top: ${containerRect.top - verticalPadding}px;
            left: ${containerRect.left - horizontalPadding}px;
            width: ${containerRect.width + horizontalPadding * 2}px;
            height: ${containerRect.height + verticalPadding * 2}px;
        ...
        const updatePosition = () => {
            ...
            this.adOverlay.style.top = (newRect.top - verticalPadding) + 'px';
            this.adOverlay.style.left = (newRect.left - horizontalPadding) + 'px';
            this.adOverlay.style.width = (newRect.width + horizontalPadding * 2) + 'px';
            this.adOverlay.style.height = (newRect.height + verticalPadding * 2) + 'px';
        };
